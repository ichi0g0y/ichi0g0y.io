...
## main反映依頼時（/deploy-to-production, /dtp 相当）

```text
- `develop -> main` の反映を実施してください。
- `base=main` / `head=develop` のOpen PRがあれば再利用し、なければ作成してください。
- `--no-merge` の指定がない限り、チェック成功後にマージまで実行してください。
- 実行後はPR URL、マージ有無、未実施項目を日本語で報告してください。
```

## staging反映依頼時（/deploy-to-staging, /dts 相当）

```text
- `develop -> staging` の反映を実施してください。
- `base=staging` / `head=develop` のOpen PRがあれば再利用し、なければ作成してください。
- `--no-merge` の指定がない限り、チェック成功後にマージまで実行してください。
- 実行後はPR URL、マージ有無、未実施項目を日本語で報告してください。
```
