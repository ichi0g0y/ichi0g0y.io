...
2. `origin/main` と `origin/develop` を最新化し、`develop` が存在することを確認する。
3. `base=main` / `head=develop` のOpen PRが既にあるか確認する。
4. Open PRがない場合は `develop -> main` のPRを作成する。
